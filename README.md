...
- ⏱️ **新增 Auto 自动翻页模式**：长按A切换到 Auto 模式后，短按B开始/停止自动执行所选模式的B键动作，短按C同理执行C键动作
- 🎯 **可选执行键位模式**：支持 page/arrow/media/music/play/custom 全部 6 种键位模式，Web 页面自由选择
- 🎲 **随机延时**：翻页间隔基础上可叠加 0~自定义上限的随机延时，模拟真人翻页节奏
- 🔄 **B/C 互斥切换**：B 执行中按 C 停止 B 并立即执行 C，反之亦然；启动后立即执行一次再按间隔循环
- 🔋 **省电优化**：WiFi 开启 modem sleep（KO 模式 STA 空闲电流大幅下降）；纯蓝牙模式 CPU 动态降到 80MHz，开启 WiFi 时自动升回 160MHz
- 🔌 **外设省电**：摇晃功能关闭时加速度计（SC7A20）进入掉电模式，不再空跑 100Hz 采样（摇色子等游戏会自动恢复采样，不影响使用）；休眠时关闭 UART 并把点阵 TX/RX 保持确定电平（空闲高/上拉），避免协控芯片 RX 浮空收噪声
- 📶 **无线发射功率可调**：Web 页新增"无线功率设置"，蓝牙发射功率默认 -6dBm（近距离手持省电，可选 -12~+9dBm）；WiFi 发射功率按 KO 子模式分开设置——AP 子模式默认 5dBm（手机直连热点）、STA 子模式默认 15dBm（连路由器），KO 模式长按 A 切换 AP/STA 时功率自动跟随，立即生效无需重启
- 📡 **蓝牙广播间隔可调**：Web 页可选 秒连(20-40ms，默认)/平衡(250ms)/省电(500ms)；未连接待机时广播是射频耗电大头，选"省电"档待机电流可降一半以上，代价是手机重连稍慢
- 😴 **可选深度睡眠**：Web 页"睡眠模式"可选深度睡眠，休眠电流从约 130µA 降到约 10µA；代价是唤醒后整机重启+蓝牙重连 3~5 秒，且仅 B/C 键可唤醒（ESP32-C3 深睡唤醒引脚限制）
- 🎮 **游戏模式 CPU 降频**：进入游戏后 CPU 降到 40MHz（BLE/WiFi 已关闭，跑游戏逻辑足够），退出自动恢复 80/160MHz
- 🛡️ **点阵通讯熔断**：协控芯片连续 3 次无响应即暂停发送（每 30 秒探测一次自动恢复），修复原先单次图标显示最坏阻塞约 1.6 秒、把按键整段吞掉的问题
- 🔀 **可开关**：Auto 模式可在"模式启用设置"里开启/关闭，关闭后长按A切换模式自动跳过
- 🔖 **KO 模式返回目标持久化**：修复 KO 模式下重启（Web 保存重启/深度睡眠唤醒/断电）后 A+B 退出 KO 回不到之前模式的问题，现在重启后也能正确返回
- 🛠️ **稳定性修复**：修复 NVS 模式值越界风险、KO 模式休眠唤醒后 WiFi 不恢复、A+B 组合期间误触发 C 键动作、长时间运行（约 49.7 天）后图标不自动熄灭、Web 休眠秒数过大溢出等问题
...
- ✅ Auto 自动翻页模式（定时执行所选模式的 B/C 键动作，B/C 键独立启停，支持随机延时）
...
- ✅ Web 配置页面（摇晃参数、模式启用、游戏开关、休眠时间与睡眠模式、无线功率、WiFi STA、KOReader 配置）
...
- 模式图标：字母 A
- 通过 Web 页面"自动翻页设置"选择要执行的键位模式（page/arrow/media/music/play/custom，不含 KOReader）
- **短按 B**：开始/停止自动执行所选模式的 B 键动作
- **短按 C**：开始/停止自动执行所选模式的 C 键动作
- B/C 互斥：B 执行中按 C 会停止 B 并立即开始执行 C，反之亦然；再次按同一个键则停止
- 按键启动后**立即执行一次**对应的按键操作，之后按设定间隔自动执行
- 每次触发显示对应动作图标（翻页箭头/音量/播放等）500ms，受"翻页箭头显示开关"控制
- 长按 C 对调翻页方向同样影响自动翻页的方向
- 翻页间隔（500ms-60分钟）和随机延时上限（0-10分钟）在 Web 页面设置，随机延时每次触发独立随机
- BLE 未连接时不执行动作（保持计时，连接后恢复）
- 自动翻页期间设备不会进入休眠；切换模式/进入游戏/休眠会自动停止
- 可在"模式启用设置"里开启/关闭，关闭后长按A切换模式时跳过
...
| 2 | 摇色子 | 摇动设备或任意键 | 标准色子1-6点，进入游戏自动恢复加速度计采样 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| 启用摇晃 | 关闭 | 实测易误触发，需要时在 Web 页开启 |
| 检测模式 | 0（三轴差值之和） | 0=三轴之和，1=各轴独立阈值 |
| 摇晃阈值 | 4000 | 三轴差值之和超过此值算摇晃中（仅模式0，范围500-50000） |
| X轴阈值 | 0 | X轴差值超过此值算摇晃中（仅模式1，0=忽略此轴，范围200-30000） |
| Y轴阈值 | 0 | Y轴差值超过此值算摇晃中（仅模式1，0=忽略此轴，范围200-30000） |
| Z轴阈值 | 2000 | Z轴差值超过此值算摇晃中（仅模式1，0=忽略此轴，范围200-30000） |
| 最小摇晃时长 | 100ms | 摇晃至少持续这么久才算有效摇晃 |
| 最大摇晃时长 | 800ms | 摇晃持续超过就算误操作，不触发翻页 |
| 静止时长 | 300ms | 摇晃结束后静止这么久才触发翻页 |
| 冷却时间 | 800ms | 触发翻页后经过冷却时间才能再次触发 |
...
- 睡眠模式可在 Web 页面选择（默认轻度睡眠）：
  - **轻度睡眠**（`esp_light_sleep_start()`）：休眠电流约 130µA，A/B/C 任意键唤醒，唤醒后快速恢复
  - **深度睡眠**（`esp_deep_sleep_start()`）：休眠电流约 10µA，仅 B/C 键可唤醒（ESP32-C3 深睡唤醒引脚限制，A=GPIO10 不支持），唤醒后整机重启并重连蓝牙（3~5 秒）
- 进入休眠前闪烁 2 次 X 图标提示
- 使用 ESP32 真正的轻量级睡眠（`esp_light_sleep_start()`），最大程度省电
- 轻度睡眠模式下休眠时关闭屏幕、蓝牙、WiFi，系统完全暂停
- 轻度睡眠通过 GPIO 唤醒：单击 A/B/C 任意按键即可唤醒（高电平触发）
...
4. **休眠设置**
   - 休眠超时时间（>=30秒，0=不休眠）
   - 睡眠模式：轻度睡眠（默认，任意键唤醒）/ 深度睡眠（约10µA，唤醒后重启+蓝牙重连，仅B/C键可唤醒）
   - 进入休眠前闪烁2次X图标提示
   - 单击A/B/C任意按键即可唤醒
   - AP有设备连接时不进入休眠

5. **无线功率设置**
   - 蓝牙发射功率（-12~+9dBm，默认 -6dBm）
   - 蓝牙广播间隔档位（秒连/平衡/省电，默认秒连）
   - WiFi 发射功率-KO AP子模式（默认 5dBm）
   - WiFi 发射功率-KO STA子模式（默认 15dBm）
...
7. **自动翻页设置（Auto模式）**
   - 执行键位模式选择（page/arrow/media/music/play/custom）
   - 翻页间隔（500-3600000ms）
   - 随机延时上限（0-600000ms，每次触发额外加 0~该值 的随机延时）
...
├── AudioDriver.h/.cpp         # 协控芯片音频/地址扫描实验代码（未被主程序引用）
└── README.md                  # 说明文档
```
...
6. 串口调试可用（USB CDC，115200），BLE 功率设置等关键操作会输出串口日志；点阵屏幕显示作为免连电脑的补充手段
